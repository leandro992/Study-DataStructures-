...
public class CountElementsGreaterThanPreviousAverage {
    public static void main(String[] args) {
        System.out.println(countGreaterThanPrevAverage(List.of(1,2)));
    }

    static int countGreaterThanPrevAverage(List<Integer> responseTimes) {
        if (responseTimes.size() <= 1) return 0;
        int count = 0;
        double sum = responseTimes.get(0); // começa com o primeiro elemento

        for (int i = 1; i < responseTimes.size(); i++) {
            double average = sum / i; // média dos anteriores (i elementos antes do atual)
            if (responseTimes.get(i) > average) {
                count++;
            }
            sum += responseTimes.get(i); // adiciona o atual para a próxima iteração
        }
        return count;
    }
}
